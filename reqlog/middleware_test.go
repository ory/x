...
func TestMiddleware_ServeHTTP(t *testing.T) {
	...
	assert.JSONEq(t,
		fmt.Sprintf(`{"http_request":{"headers":{"x-request-id":["22035D08-98EF-413C-BBA0-C4E66A11B28D"],"x-real-ip":["10.10.10.10"]},"host":"example.com","method":"GET","path":"/stuff","query":"Value is sensitive and has been redacted. To see the value set config key \"log.leak_sensitive_values = true\" or environment variable \"LOG_LEAK_SENSITIVE_VALUES=true\".","remote":"","scheme":"http"},"level":"info","msg":"started handling request","time":"%s"}`, nowToday),
		lines[0], lines[0])
	assert.JSONEq(t,
...
func TestMiddleware_ServeHTTP_AfterOverride(t *testing.T) {
	...
	assert.Len(t, lines, 2)
	assert.JSONEq(t,
		fmt.Sprintf(`{"hambone":57,"http_request":{"headers":{"x-request-id":["22035D08-98EF-413C-BBA0-C4E66A11B28D"],"x-real-ip":["10.10.10.10"]},"host":"example.com","method":"GET","path":"/stuff","query":"Value is sensitive and has been redacted. To see the value set config key \"log.leak_sensitive_values = true\" or environment variable \"LOG_LEAK_SENSITIVE_VALUES=true\".","remote":"","scheme":"http"},"level":"info","msg":"completed handling request","time":"%s"}`, nowToday),
		lines[1], lines[1])
}
