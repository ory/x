...
func (f *FetcherNext) ResolveKeyFromLocations(ctx context.Context, locations []string, modifiers ...FetcherNextOption) (jwk.Key, error) {
	...
	for k := range locations {
		...
		eg.Go(func() error {
			...
			for i := 0; i < remoteSet.Len(); i++ {
				key, found := remoteSet.Key(i)
				if !found {
					return errors.WithStack(herodot.ErrBadRequest.WithReason("failed to get key from JWK set"))
				}

				if err := set.AddKey(key); err != nil {
					return errors.WithStack(herodot.ErrBadRequest.WithReason("failed to add key to JWK set").WithWrap(err))
				}
			}
