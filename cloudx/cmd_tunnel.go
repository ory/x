...
func NewTunnelCommand(project string, self string, version string) *cobra.Command {
	proxyCmd := &cobra.Command{
		...
		Short: fmt.Sprintf("Tunnel Ory %s on a subdomain of your app or a seperate port your app's domain", project),
		Args:  cobra.RangeArgs(1, 2),
		Long: fmt.Sprintf(`This command starts an HTTP tunnel to Ory %[2]s on the domain (<[tunnel-url]>) you specify. This
...
Before you start, you need to have a running instance of Ory %[2]s. Set the environment variable
ORY_SDK_URL to the path where Ory %[2]s is available. For Ory Cloud, this is the "SDK URL"
